...
def get_args_parser():
    ...
    parser.add_argument(
        "--disable_weight_decay_on_cls_pos", action="store_true", default=False
    )
    parser.add_argument(
        "--disable_weight_decay_on_bias_norm", action="store_true", default=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="which head layer to start to finetune",
    )
    parser.add_argument(
        "--out_dim",
        default=8192,
        type=int,
        help="only useful when finetune all the projection layers",
    )
...
class CarsDataset(ImageFolder):
    def __init__(
        ...
        self.loader = loader
        self.target_transform = target_transform
        data = scio.loadmat(os.path.join(root, f"cars_annos.mat"))["annotations"][
            0
        ].tolist()
...
class FlwrsDataset(ImageFolder):
    def __init__(
        ...
        ids = (
            np.concatenate((ids["trnid"], ids["valid"]), axis=1)[0]
            if train
            else ids["tstid"][0]
        )
...
def main(args):
    ...
    if args.finetune_head_layer > 0:
        ...
        head = (
            nn.Linear(embed_dim, args.nb_classes)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (test_stats["acc1"] >= max_accuracy):
            ...
            checkpoint_paths = [
                output_dir / "checkpoint_{}_cls.pth".format(args.checkpoint_key)
            ]
