...
from tools.model_utils import adv_bgs_eval_model, make_and_restore_model
...
def main(args):
    ...
    if checkpoint is None:
        model, _ = make_and_restore_model(
            arch=arch, dataset=train_ds, pytorch_pretrained=True
        )
    else:
        model, _ = make_and_restore_model(
            arch=arch, dataset=train_ds, resume_path=checkpoint
        )
    model.cuda()
    model.eval()
    ...
    for fg_class in range(9):
        ...
        for i in range(len(fgs)):
            if total_computed % 50 == 0:
                cur_time = time.time()
                print(
                    f"At image {i} for class {fg_classname}, used {(cur_time-prev_time):.2f} since the last print statement."
                )
                print(
                    f"Up until now, have {total_vulnerable}/{total_computed} vulnerable foregrounds."
                )
                prev_time = cur_time
    ...
    print(
        f"Summary: {total_vulnerable}/{total_computed} ({percent_vulnerable:.2f}%) are vulnerable foregrounds."
    )
