...
def make_and_restore_model(
    *_,
    ...
    if pytorch_pretrained:
        ...
        classifier_model = (
            dataset.get_model(arch, pytorch_pretrained)
    ...
    if resume_path:
        if os.path.isfile(resume_path):
            ...
            sd = {
                k[len("module.") :] if ("module." in k) else k: v for k, v in sd.items()
            }
...
def adv_bgs_eval_model(
    bg_loader, model, im, mask, fg_class, batch_size, map_to_in9, map_in_to_in9=True
):
    """
    *Internal function*
    Args:
