...
def main():
    ...
    parser.add_argument(
        "--local_rank", default=-1, type=int, help="node rank for distributed parallel"
    )
    parser.add_argument(
        "--arch",
        default="vit_small",
    ...
    parser.add_argument(
        "--pretrained_path", type=str, default="", help="the pretraining models"
    )
    parser.add_argument(
        "--checkpoint_key",
        default="teacher",
    ...
    parser.add_argument(
        "--save_path", type=str, default="", help="where to save the memory_bank"
    )
    parser.add_argument("--batch_size", type=int, default=32, help="batch size")
    parser.add_argument("--patch_size", type=int, default=16, help="patch size")
    parser.add_argument("--img_size", type=int, default=224, help="image size")
    parser.add_argument(
        "--patch_window", type=int, default=5, help="patch visualize window"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="show pics of topk cluster with most items",
    )
    parser.add_argument(
        "--chunks",
        type=int,
        default=16,
        help="""Number of counting chunks. Set this larger (e.g., 128
    ...
    network = utils.MultiCropWrapper(
        network,
        iBOTHead(
            network.embed_dim,
            args.out_dim,
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        train_dataset, shuffle=False, rank=args.local_rank
    )
    ...
    try:
        ...
        for data in tqdm(train_dataloader):
            ...
            feature = (
                network(img)[1].contiguous()
                if args.type == "patch"
                else network(img)[0].contiguous()
            )
            feature = concat_all_gather(feature).detach().cpu()
            idx = concat_all_gather(idx)
            if memory_bank is None:
                print(
                    "Initializing memory_bank bank: {} points.".format(n_train_points)
                )
                memory_bank = (
                    torch.zeros(n_train_points, feature.size(1), 2)
                    if args.type == "patch"
                    else torch.zeros(n_train_points, 2)
                )
    ...
    if num_per_cluster is None and args.local_rank == 0:
        ...
        torch.save(
            {"memory_bank": memory_bank, "num_per_cluster": num_per_cluster},
            os.path.join(args.save_path, f"memory_{args.type}.pth"),
        )
