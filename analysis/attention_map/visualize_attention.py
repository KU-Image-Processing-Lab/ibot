...
def apply_mask2(image, mask, color, alpha=0.5):
    ...
    for c in range(3):
        image[:, :, c] = (
            image[:, :, c] * (1 - alpha * np.sqrt(mask) * (mask > t))
            + alpha * np.sqrt(mask) * (mask > t) * color[c] * 255
        )
    return image
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_path", default=None, type=str, help="Path of the single image."
    )
    parser.add_argument(
        "--data_path",
    ...
    def show_attn_color(
        image, attentions, th_attn, index=None, head=[0, 1, 2, 3, 4, 5]
    ):
        M = image.max()
        m = image.min()
        span = 64
        ...
        for i in range(N):
            ...
            if contour:
                padded_mask = np.zeros(
                    (_mask.shape[0] + 2, _mask.shape[1] + 2)
                )  # , dtype=np.uint8)
                padded_mask[1:-1, 1:-1] = _mask
                contours = find_contours(padded_mask, 0.5)
                for verts in contours:
    ...
    if args.image_path is not None and os.path.isfile(args.image_path):
        ...
        pic_attn_color = show_attn_color(
            img.permute(1, 2, 0).cpu().numpy(), attentions, th_attn
        )
        final_pic = Image.new(
            "RGB", (pic_i.size[1] * 2 + pic_attn.size[0], pic_i.size[1])
        )
        ...
        for data in tqdm(train_dataloader):
            ...
            for i in range(img.size(0)):
                attentions, th_attn, pic_i, pic_attn = show_attn(
                    img[i : i + 1], index=idx[i].item()
                )
                pic_attn_color = show_attn_color(
                ...
                final_pic.paste(pic_attn, (pic_i.size[1] * 2, 0))
                final_pic.save(
                    os.path.join(args.output_dir, f"idx{idx[i].item()}_attn.png")
                )
                cnt += 1
