...
def eval_linear(args):
    ...
    model = vit_dict[args.arch](
        patch_size=args.patch_size, num_classes=0, use_mean_pooling=True
    )
    model.cuda()
    ...
    args.lrs = [
        base * n for base in [10**k for k in range(-4, 1)] for n in range(1, 10)
    ]
    ...
    for pm in args.permutes:
        ...
        linear_classifier = LinearClassifier(
            model.embed_dim, num_labels=args.num_labels
        )
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train_stats = train(
            model, linear_classifiers, optimizers, train_loader, epoch, args.permutes
        )
        ...
        if epoch % args.val_freq == 0 or epoch == args.epochs - 1:
            ...
            test_stats = validate_network(
                val_loader, model, linear_classifiers, args.permutes
            )
            ...
            for group, pm in enumerate(args.permutes):
                ...
                if group % (len(args.wds) * len(args.optims)) == 0:
                    group_best_acc_sweep_lr_only = max(
                        group_best_acc_sweep_lr_only,
            ...
            best_acc_sweep_lr_only = max(
                best_acc_sweep_lr_only, group_best_acc_sweep_lr_only
            )
            print(f"Max accuracy so far: {best_acc:.2f}%")
            print(
                f"Max accuracy with sweeping lr only so far: {best_acc_sweep_lr_only:.2f}%"
            )
...
def train(model, linear_classifiers, optimizers, loader, epoch, permutes):
    ...
    for group, _ in enumerate(permutes):
        metric_logger.add_meter(
            "lr{}".format(group), utils.SmoothedValue(window_size=1, fmt="{value:.5f}")
        )
    ...
    for inp, target in metric_logger.log_every(loader, 20, header):
        ...
        # forward
        with torch.no_grad():
            intermediate_output, attentions = (
                model.get_intermediate_layers_and_last_selfattention(inp, 1)
            )
            # we keep only the output patch attention
            attentions = attentions[:, :, 0, 1:].mean(1) * (
                -1 if args.reverse_top else 1
            )
        ...
        for group, (pm, loss, optimizer) in enumerate(
            zip(permutes, losses, optimizers)
        ):
            metric_logger.update(**{"loss{}".format(group): loss.item()})
            metric_logger.update(
                **{"lr{}".format(group): optimizer.param_groups[0]["lr"]}
            )
...
def validate_network(val_loader, model, linear_classifiers, permutes):
    ...
    for inp, target in metric_logger.log_every(val_loader, 20, header):
        ...
        acc1s = []
        acc5s = []
        for group, (pm, linear_classifier) in enumerate(
            zip(permutes, linear_classifiers)
        ):
            pred = linear_classifier(output)
            loss = nn.CrossEntropyLoss()(pred, target)
            losses.append(loss)
            ...
            metric_logger.update(**{"loss{}".format(group): loss.item()})
            metric_logger.meters["acc{}@1".format(group)].update(
                acc1.item(), n=batch_size
            )
            if linear_classifier.module.num_labels >= 5:
                metric_logger.meters["acc{}@5".format(group)].update(
    ...
    for group, (pm, linear_classifier) in enumerate(zip(permutes, linear_classifiers)):
        ...
        if linear_classifier.module.num_labels >= 5:
            print(
                "* [Lr {lr:.5f} Wd {wd:.0e} Optim {optim:4}] Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}".format(
                    lr=lr,
                    wd=wd,
                    optim=optim,
            ...
            print(
                "* [Lr {lr:.5f} Wd {wd:.0e} Optim {optim:4}] Acc@1 {top1.global_avg:.3f} loss {losses.global_avg:.3f}".format(
                    lr=lr,
                    wd=wd,
                    optim=optim,
                    top1=metric_logger.meters["acc{}@1".format(group)],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=4,
        type=int,
        help="Only store the top k patch tokens for each head with highest attention",
    )
    parser.add_argument(
        "--reverse_top", default=False, type=bool, help="Change topk to bottomk"
    )
