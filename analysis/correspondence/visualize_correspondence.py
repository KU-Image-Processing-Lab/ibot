...
def main():
    ...
    parser.add_argument(
        "--pretrained_weights", type=str, default="", help="the pretraining models"
    )
    parser.add_argument("--batch_size", type=int, default=2, help="batch size")
    parser.add_argument("--patch_size", type=int, default=16, help="patch size")
    parser.add_argument("--img_size", type=int, default=224, help="image size")
    parser.add_argument(
        "--output_path", type=str, default="pics/", help="patch for output images"
    )
    ...
    print(
        "We load the reference pretrained DINO weights to extract self-attention for fair comparison."
    )
        ...
        class Augmentation(object):
            def __init__(self, img_size):
                ...
                self.global_transfo1 = transforms.Compose(
                    [
                        RandomResizedCrop(
                            img_size, scale=(0.4, 1.0), interpolation=Image.BICUBIC
                        ),
    ...
    if args.sample_type == "class":
        ...
        train_dataset = ImageFolderInstance(
            args.data_path, transform=lambda x: torch.Tensor(0)
        )
        ...
        train_dataloader = DataLoader(
            train_dataset, batch_sampler=batch_sampler, num_workers=4, pin_memory=True
        )
    else:
        train_dataset = ImageFolderInstance(args.data_path, transform=augmentation)
        train_dataloader = DataLoader(
    ...
    for data in tqdm(train_dataloader):
        if args.sample_type == "class":
            ...
            pos1 = (
                torch.stack(
                    (
                        torch.from_numpy(
                            np.linspace(pos1[:, 0], pos1[:, 2], num=2 * unit + 1)[1::2]
                        )
                        .unsqueeze(1)
                        .expand(-1, unit, -1),
                        torch.from_numpy(
                            np.linspace(pos1[:, 1], pos1[:, 3], num=2 * unit + 1)[1::2]
                        )
                        .unsqueeze(0)
                        .expand(unit, -1, -1),
                    )
            ...
            pos2 = (
                torch.stack(
                    (
                        torch.from_numpy(
                            np.linspace(pos2[:, 0], pos2[:, 2], num=2 * unit + 1)[1::2]
                        )
                        .unsqueeze(1)
                        .expand(-1, unit, -1),
                        torch.from_numpy(
                            np.linspace(pos2[:, 1], pos2[:, 3], num=2 * unit + 1)[1::2]
                        )
                        .unsqueeze(0)
                        .expand(unit, -1, -1),
                    )
            ...
            ovlp = ((pos1 + eps) > pos2.min(dim=1, keepdim=True)[0]).all(dim=-1) & (
                (pos1 - eps) < pos2.max(dim=1, keepdim=True)[0]
            ).all(dim=-1)
        ...
        attentions = attentions[:, :, 0, 1:].view(
            x1.size(0), -1, args.img_size // 8, args.img_size // 8
        )
        attentions1 = nn.functional.interpolate(
            attentions, scale_factor=0.5, mode="nearest"
        )
        if ovlp is not None:
            attentions1 = attentions1 * ovlp.float()
            attentions = attentions * nn.functional.interpolate(
                ovlp.float(), scale_factor=2, mode="nearest"
            )
        ...
        if args.sample_type == "instance":
            ...
            real_dst_matrix = (
                pos1.square().sum(2, keepdim=True)
                + pos2.permute(0, 2, 1).square().sum(1, keepdim=True)
                - 2 * real_sim_matrix
            )
            ...
            accuracy = (
                ((index_h - real_index_h).abs() <= args.eval_radius)
                & ((index_w - real_index_w).abs() <= args.eval_radius)
                & ovlp.flatten(1)
            ).sum() / ovlp.sum()
            acc.append(accuracy)
        ...
        if args.show_pics:
            for (
                ...
                attention,
                attention1,
            ) in zip(cls, index, value, img1, img2, attentions, attentions1):
                ...
                for nh in range(attention.size(0)):
                    ...
                    if args.show_with_attn:
                        ...
                        plt.imsave(
                            fname=f"{args.output_path}/temp_attn.png",
                            arr=attn,
                            format="png",
                        )
                        img.paste(
                            Image.open(f"{args.output_path}/temp_attn.png"), (0, 0)
                        )
                    ...
                    for p1, p2 in zip(point1, point2):
                        ...
                        draw.line(
                            (
                                p1x * args.patch_size
                                + args.img_size * (1 if args.show_with_attn else 0),
                                p1y * args.patch_size,
                                p2x * args.patch_size
                                + args.img_size * (2 if args.show_with_attn else 1),
                                p2y * args.patch_size,
                            ),
                            width=1,
                            fill="red",
                    ...
                    img.save(
                        f"{args.output_path}/{args.sample_type}_corresp{cnt}_cls{cs}_nh{nh}.png"
                    )
