...
def main(args, device, verbose=True):
    ...
    if verbose:
        if args.shuffle:
            ...
            print(
                f"{args.model_name} dropping {args.drop_count} random patches from block {args.block_index}"
            )
        elif args.cascade:
            print(f"evaluating {args.model_name} in cascade mode")
        elif args.saliency:
            print(
                f"{args.model_name} dropping {'most' if args.drop_best else 'least'} "
                f"salient {args.drop_count} patches"
            )
        elif args.saliency_box:
            print(
                f"{args.model_name} dropping {args.drop_lambda} % most salient pixels"
            )
        elif args.standard_box:
            print(
                f"{args.model_name} dropping {args.drop_lambda} % pixels around most matching patch"
            )
        elif args.dino:
            print(
                f"{args.model_name} picking {args.drop_lambda * 100} %  "
                f"{'foreground' if args.drop_best else 'background'} pixels using dino"
            )
        else:
            print(
                f"{args.model_name} dropping {'least' if args.drop_best else 'most'} "
    ...
    if args.dino:
        ...
        dino_model = dino.dino_small(
            patch_size=vars(args).get("patch_size", 16), pretrained=True
        )
        dino_model.to(device)
        dino_model.eval()

    if "vit" in args.model_name:
        model = imagenet_models.__dict__[args.model_name](
            num_classes=1000, pretrained=False, linear_eval=args.linear_eval
        )
            ...
            if os.path.isfile(args.pretrained_weights):
                ...
                print(
                    "=> loaded checkpoint '{}' (epoch {})".format(
                        args.pretrained_weights, checkpoint["epoch"]
                    )
                )
                ...
                error_msg = "=> no checkpoint found at '{}'".format(
    ...
    if verbose:
        print(
            f"Parameters in Millions: {sum(p.numel() for p in model.parameters() if p.requires_grad) / 1000000:.3f}"
        )
    ...
    color = (
        ...
        if args.imagenet_default_color
            ...
            if args.shuffle or args.random_drop:
                ...
                if args.shuffle:
                    row = np.random.choice(
                        range(patch_num), size=img.shape[1], replace=False
                    )
                    img = img[:, row, :]  # images have been shuffled already
                elif args.random_drop and args.drop_count > 0:
                    row = np.random.choice(
                        range(patch_num), size=args.drop_count, replace=False
                    )
                    if args.random_offset_drop:
                        mask[:, row, :] = 0.0
                    else:
                ...
                if args.random_offset_drop and args.drop_count > 0:
                    ...
                    mask_off_set = 8
                    new_mask[:, :, mask_off_set:, mask_off_set:] = mask[
                        :, :, :-mask_off_set, :-mask_off_set
                    ]
                    img = new_mask * img
                ...
                th_attn = torch.nn.functional.interpolate(
                    th_attn.unsqueeze(1), scale_factor=scale, mode="nearest"
                )
    ...
    if args.dino:
        print(
            f"{args.model_name} Top-1 Accuracy: {clean_acc / len(test_set)} with information loss ratio {pixel_percent / len(test_set)}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", type=str, default="deit_small_patch16_224", help="Model Name"
    )
    ...
    parser.add_argument(
        "--drop_count", type=int, default=180, help="How many patches to drop"
    )
    parser.add_argument(
        "--drop_best",
        action="store_true",
        default=False,
        help="set True to drop the best matching",
    )
    parser.add_argument(
        "--test_image",
        action="store_true",
        default=False,
        help="set True to output test images",
    )
    parser.add_argument(
        "--shuffle",
        action="store_true",
        default=False,
        help="shuffle instead of dropping",
    )
    parser.add_argument(
        "--shuffle_size", type=int, default=14, help="nxn grid size of n", nargs="*"
    )
    parser.add_argument(
        "--shuffle_h", type=int, default=None, help="h of hxw grid", nargs="*"
    )
    parser.add_argument(
        "--shuffle_w", type=int, default=None, help="w of hxw grid", nargs="*"
    )
    parser.add_argument(
        "--random_drop",
        action="store_true",
        default=False,
        help="randomly drop patches",
    )
    parser.add_argument(
        "--random_offset_drop",
        action="store_true",
    ...
    parser.add_argument(
        "--cascade", action="store_true", default=False, help="run cascade evaluation"
    )
    parser.add_argument(
        "--exp_count",
        type=int,
    ...
    parser.add_argument(
        "--saliency", action="store_true", default=False, help="drop using saliency"
    )
    parser.add_argument(
        "--saliency_box", action="store_true", default=False, help="drop using saliency"
    )
    parser.add_argument(
        "--drop_lambda",
        type=float,
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="drop using standard model",
    )
    parser.add_argument(
        "--dino",
        action="store_true",
        default=False,
        help="drop using dino model saliency",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="block index for lesion method",
        nargs="*",
    )

    parser.add_argument(
        "--draw_plots", action="store_true", default=False, help="draw plots"
    )
    parser.add_argument(
        "--select_im", action="store_true", default=False, help="select robust images"
    )
    parser.add_argument("--save_path", type=str, default=None, help="save path")

    # segmentation evaluation arguments
    parser.add_argument(
        "--threshold", type=float, default=0.9, help="threshold for segmentation"
    )
    parser.add_argument(
        "--pretrained_weights", default=None, help="pretrained weights path"
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="use shape token for prediction",
    )
    parser.add_argument(
        "--rand_init",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="generate images instead of eval",
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Wether to use linear eval model or finetune model",
    )
    parser.add_argument(
        "--imagenet_default_color",
        type=utils.bool_flag,
        default=True,
        help="""We do not use default imnet color for 
    ...
    parser.add_argument(
        ...
        type=utils.bool_flag,
        default=False,
        help="""We use bicubic interpolation to resize for 
        finetuned models from BEiT repo.""",
    )
    ...
    if opt.shuffle:
        if opt.shuffle_h is not None:
            assert (
                opt.shuffle_w is not None
            ), "need to specify both shuffle_h and shuffle_w!"
            assert len(opt.shuffle_h) == len(
                opt.shuffle_w
            ), "mismatch for shuffle h, w pairs"
        ...
        for rand_exp in range(opt.exp_count):
            ...
            for drop_count in range(0, 10):
                if isinstance(opt.shuffle_size, list):
                    opt.drop_count = (
                        drop_count * opt.shuffle_size[0] * opt.shuffle_size[1] // 10
                    )
                else:
